  ...
  if (error) {
    ...
    try {
      ...
      const res = await db.query(
        "SELECT initialized FROM initial_setup WHERE id = 1"
      );
      const initialized = res.rows[0]?.initialized;

      if (!initialized) {
        await db.query(
          `INSERT INTO initial_setup (id,initialized) VALUES (1,FALSE)`
        );
        ...
        console.log(
          "Datos iniciales en tabla productos insertados correctamente"
        );
        ...
        console.log(
          "Datos iniciales en tabla categorias insertados correctamente"
        );
        ...
        await db.query(
          "UPDATE initial_setup SET initialized = TRUE WHERE id = 1"
        );
